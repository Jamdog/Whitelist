 ...
 * @author Stefan Cole (aka. Jamdoggy) <jamdog@live.co.uk> Edited by Paul Corey (aka. FamilyCraft_Dad)
...
class whitelistUser {
	...
	public $wynncraftAPI = false;  //Toggles anything relating to wynnCraft
		...
		if (isset($uid) && ($uid != NULL)) {
			...
			if ($this->userData['premium'] != 'true') {
				if ($this->userData['premium'] == 'false') {
					$this->e['title'] = "Error: Player does not have a premium minecraft account.";
					$this->e['text'] = "Player does not have a premium minecraft account.<br />";
					//$this->e['code'] = ERR_API_NOT_PREMIUM;
				} else {
	...
	private function getUUID($uid=NULL) {
		...
		if (isset($uid) && ($uid != NULL)) {
			...
			if (!isset($uuidData)) {
				...
				$this->e['text'] .= "Press F5 to refresh, if you wish to try again.";
				//$this->e['code'] = ERR_API_UUID_FAILURE;
			} else {
				$uuidData = json_decode($uuidData, true);
				$this->userData['UUID'] = $uuidData['id'];
				$this->userData['formattedUUID'] = $this->formatUUID($uuidData['id']);
			}
		} else {
			$this->userData['UUID'] = NULL;
			$this->e['title'] = "Warning: Invalid Username or IGN not set";
	...
	private function getUserData($uid=NULL) {
		...
		if (isset($uid) && ($uid != NULL)) {
			...
			if ($this->wynncraftAPI == true && (empty($playerData) || !isset($playerData))) {
